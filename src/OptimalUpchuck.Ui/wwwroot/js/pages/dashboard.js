...
function initDonutChart(element) {
    ...
    const keyHealthData = generateRandomData();
            ...
            series: [
                {
                    ...
                    data: keyHealthData.map((item, index) => ({
                        value: item.value,
                        itemStyle: {
...
for (let i = -14; i <= 0; i++) {
    ...
    // Random order counts
    let weight = Math.floor(Math.random() * 200);
    let calories = Math.floor(Math.random() * 150);
    let carbs = Math.floor(Math.random() * 50);

    weightInPounds.push(weight);
    caloriesCounted.push(calories);
    carbsCounted.push(carbs);
}
